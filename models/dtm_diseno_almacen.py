...
class Materiales(models.Model):
    ...
    def insertar(self,cantidad,nombre,medida,id,existe):
            if existe:
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        for lamina in get_lamina:
            ...
            nombre = lamina.material_id.nombre + " "
            medida = str(lamina.largo) + " x " + str(lamina.ancho) + " @ " + str(lamina.calibre)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(lamina.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for angulo in get_angulos:
            nombre = angulo.material_id.nombre
            medida = str(angulo.alto) + " x " + str(angulo.ancho) + " @ " + str(angulo.calibre) +", " + str(angulo.largo)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(angulo.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for canal in get_canal:
            nombre = canal.material_id.nombre
            medida = str(canal.alto) + " x " + str(canal.ancho) + " espesor " + str(canal.espesor) +", " + str(canal.largo)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(canal.cantidad),nombre,medida,str(id),get_info!=None)
            # print(nombre)
            id += 1

        for perfiles in get_perfiles:
            nombre = perfiles.material_id.nombre
            medida = str(perfiles.alto) + " x " + str(perfiles.ancho) + " @ " + str(perfiles.calibre) +", " + str(perfiles.largo)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(perfiles.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for pintura in get_pintura:
            nombre = pintura.material_id.nombre
            medida = str(pintura.cantidades)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(pintura.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1
        ...
        for rodamientos in get_rodamientos:
            ...
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(rodamientos.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for solera in get_solera:
            nombre = solera.material_id.nombre
            medida = str(solera.largo) + " x " + str(solera.ancho) + " @ " + str(solera.calibre)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(solera.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1
        ...
        for tornillos in get_tornillos:
            ...
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(tornillos.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for tubos in get_tubos:
            nombre = tubos.material_id.nombre
            medida = str(tubos.diametro) + " x " + str(tubos.largo) + " @ " + str(tubos.calibre)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(tubos.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1

        for varilla in get_varilla:
            nombre = varilla.material_id.nombre
            medida = str(varilla.diametro) + " x " + str(varilla.largo)
            get_info = self.env['dtm.diseno.almacen'].search([("id","=",id)])
            self.insertar(str(varilla.cantidad),nombre,medida,str(id),get_info!=None)
            id += 1
